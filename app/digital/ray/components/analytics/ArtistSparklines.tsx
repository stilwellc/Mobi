...
function SparkTooltip({ active, payload }: { active?: boolean; payload?: Array<{ payload: SparkPoint }> }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 9, color: 'var(--color-text-label)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 3 }}>
        {d.date}
      </div>
      <div style={{ fontSize: 12, color: '#96B8D4', fontWeight: 500 }}>
      ...
      <div style={{
        ...
        {artist.appreciation !== 0 && (
          <div style={{
            fontSize: 11,
            fontWeight: 600,
            color: artist.appreciation > 0 ? 'var(--color-accent-green)' : 'var(--color-accent-pink)',
            whiteSpace: 'nowrap',
          }}>
            ...
            <AreaChart data={artist.sparkData} margin={{ top: 2, right: 0, left: 0, bottom: 0 }}>
              <defs>
                <linearGradient id={`spark-${artist.slug}`} x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor={trendUp ? '#96B8D4' : '#D496B8'} stopOpacity={0.25} />
                  <stop offset="100%" stopColor={trendUp ? '#96B8D4' : '#D496B8'} stopOpacity={0.02} />
                </linearGradient>
              </defs>
              ...
              <Area
                type="monotone"
                dataKey="avgPrice"
                stroke={trendUp ? '#96B8D4' : '#D496B8'}
                strokeWidth={1.5}
                fill={`url(#spark-${artist.slug})`}
                dot={false}
                activeDot={{ r: 2, fill: trendUp ? '#96B8D4' : '#D496B8', stroke: 'var(--color-bg-card)', strokeWidth: 1.5 }}
              />
            </AreaChart>
          </ResponsiveContainer>
        </div>
    ...
    <section className="ray-sparklines" style={{ maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: 20 }}>
        <h2 style={{
          ...
          Artist <span style={{ fontStyle: 'italic', color: 'var(--color-accent-green)' }}>Performance</span>
        </h2>
      </div>
